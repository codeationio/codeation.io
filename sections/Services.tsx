...
const ServicesCard = dynamic(() => import('components/Card/ServicesCard'), {
  suspense: true,
});
...
const Services = () => (
  <Section className="grid grid-cols-2 gap-5 lg:grid-cols-3 xl:gap-16" id="services" title="Our Services">
    {cards.map((card, index) => (
      <ServicesCard key={index} description={card.description} image={card.image} title={card.title} />
    ))}
  </Section>
);
